...
sequenceDiagram
    ...
    loop 每个分段
        ...
        CA->>S: POST /v1/asr?variant=meeting&timestamps=1
...
| 桌面版本体与家目录 | 原样不动 | 原样不动 |
| 会话 / 记忆 | **不导**（见下） | 不导 |
| 面板上 | 标准版里立刻能看到那些技能 | 技能列表是空的，**并明说为什么** |

**只导 skills，不导会话/记忆。** 会话存储是 DSH 的内部格式，跨版本导入风险高，
而且导坏了用户无从判断。v1 的边界是：**技能是"能力"，可以搬；会话是"历史"，
留在原地（他打开桌面版就能看）**。面板必须把这句话说出来，否则用户会以为都搬过来了。
...
| **3** | 前后分离 §3 设计了服务端指令增强（执行原子点 + grace 窗口）；服务端文档 §14 一度建议 v1 不开 | **设计保留、默认关闭。** 服务端 `models.specs` 里 `asr-short`（SenseVoice）本来就常驻（会议档也用），所以能力在；但 `/v1/asr?variant=short` 的**默认配额为 0** = 不对外。开启只需改配额。 |
...
| 12 | **桌面版资产只导 skills、不导会话/记忆**（§6.4） | 我的判断：会话是 DSH 内部格式，跨版本导入风险高且坏了用户无从判断。**若你希望连会话一起搬，这条要改** |
