...
| 分组 `grp` | 面板标题 | 项数 | 内容 |
|---|---|---|---|
| `agent` | 智能体 | — | 智能体表格（谁干活）；该组只放 `hidden` 的智能体键 —— 包括 `dshBaseUrl`（2026-09-19 从「面板与服务」挪上来，见 §4.6） |
...
| `panel` | 面板与服务 | 7 | ECHO 端口、仪表盘热键/打开方式/自动显示/收起、自动刷新秒、API 鉴权 |
| `paths` | 存储路径 | 2 | 会议文件目录、模型目录 |
| `router` | 模型路由 | 7 | 已移到顶部「模型路由」页签（注册 / 显示名 / 探测 / 超时 / 熔断，见 §4.7；页签故障时回退显示） |
...
### 4.6 智能体自己的配置项：长在智能体那一行里

用户实测截图指出：「DSH 服务地址」摆在「面板与服务」里，与上面「智能体」表格 DSH 那行显示的
是同一个地址（那行已经写着"API 可访问（http://127.0.0.1:43120）"）—— 原话
"下面的 dsh 没必要吧，或者把端口挪上去"。

于是这类"只属于某个智能体"的键统一收进 `agent` 组并标 `hidden`，由**适配器声明归属**
（`AgentAdapter.settings_keys`：DSH → `dshBaseUrl`，CodeBuddy → `agentCustomPath`），
值经 `GET /api/agents` 的 `settings` 字段下发，面板在对应智能体的**展开区**里渲染
（与"检测"按钮、状态说明同处一地）。不变量由 `tests/test_agent_settings.py` 钉住：
键必须存在、必须 hidden、必须归 `agent` 组、不能被两个智能体声明、不许是密钥、改完能读回。

> 顺带修掉一个隐藏 bug：CodeBuddy 的「CLI 路径」原来用 `settingsValue("agentCustomPath")` 回填，
> 而 hidden 键根本不在 `/api/settings` 里 —— 那个框永远是空的（用户每次都得重填）。

### 4.7 路由参数：搬进「模型路由」页签

同一天的第二条同类反馈："设置中的模型路由标签整合到模型路由中"。
那 7 项（`router*`）本来就只有和该页签的成员/优先级放一起才看得懂
（"注册到 DSH"、"模型组显示名"就是那一页在用的东西），于是：

* 面板侧用 `ROUTER_KEYS` 把它们从设置表单里过滤掉（**前端过滤，后端 `grp` 不动**），
  「模型路由」页签新增一张「路由参数」卡（`#rtSetHost`），
  **复用设置页的 `renderSettingRow()`**（标签/控件/说明样式完全一致）+ 一个「保存」按钮
  （只收集本卡内的字段；`PUT /api/settings` 本来就会把路由项同步写进
  `dsh-failover/config.json` 并热重载）；
* 页签故障时回退：`_rtTabOk=false` 时设置页重新显示这一组（与「能力」页签同一套回退机制），
  并在页顶给一句指路；
* `ROUTER_KEYS` 必须与 `config.DEFAULTS` 里 `grp="router"` 的键一一对应（测试钉住，防漂移）；
* `scripts/preview-settings-menu.py` 同步支持 `--router-fail`，默认按面板的规则过滤。
