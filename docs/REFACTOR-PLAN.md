...
### 9.1 本机 spike 结果（2026-09-19，win-x64，runtime `0.1.5rc1`）

全部在**独立 venv + 独立 `dsh_home`**里跑，未触碰 `~/.dsh`。

| # | 结论 | 证据 / 影响 |
|---|---|---|
| **S1** | ✅ 通过 | `pip install deepseek-harness-sdk`（+ `-runtime-bin`）23.8 s。**实测占用 265 MB**（`deepseek_harness_runtime` 解包 234.6 MB）—— 清单 `size_mb` 用这个值，**不要用 wheel 的 72 MB**。`dsh 0.1.5-rc.1` 可用；**把 Node 从 PATH 里剥离后仍正常**（运行时是原生 exe，实测） |
| **S2** | ✅ 通过 | 独立 home 首次启动（含 profile 引导）**2.9 s**，一次 `run()` **1.2 s**，`finish_reason=completed`、`final_response` 正确。home 仅 **0.8 MB**。`RunResult.events`（13 条）含 `assistant/message` / `turn/start|end` / `step/*` / `session/title` / `request/context` —— **P6 的内嵌会话视图直接消费这个面** |
| **S3** | ✅ 通过 | 不挂载时 agent 答 **`NONE`**（SDK 确实不读 `~/.dsh`，与官方文档一致）；把共享技能目录挂到 **`<cwd>/.dsh/skills`** 或 **`<DSH_HOME>/skills`** 都能列出**全部 12 个技能**（含 `meeting-record`）并读取 `SKILL.md`。→ **D6 的"挂载共享技能目录"确定为普通 junction 挂载**，两个位置都行（推荐挂 home，少受 cwd 影响） |
...
只能当 best-effort，并在升级运行时之后重新验证。**不要把 D26 的验收押在 `--profile web` 上。**

**待查 / 需更正**

- 直连内网网关 `TRANSPORT` 的确切原因未定位；**只有在 2.0 确实要让 agent 直连内网时才需要继续挖**（走 ECHO 路由已够用）。
...
| 分支 | 角色 | 谁能合进来 |
|---|---|---|
| `main` | **稳定线**（v2.0.0 起也是唯一开发线）——永远处于"能装、能用"的状态 | bugfix、P0 清理、**新增配置项但默认行为不变**的改动，以及结构改动（见下方纪律 2） |
| ~~`2.0-dev`~~ | ~~长驻开发线（P3–P8）~~ | **已于 v2.0.0 发布时并入 `main` 并删除**（2026-09-23） |
| `fix/*`、`feat/*` | 短分支 | 从 `main` 切、PR 回 `main` |

纪律（2026-09-23 v2.0.0 发版后更新）：

1. **GitHub 默认分支保持 `main`** —— clone 默认拿到稳定版；
2. ~~`main → 2.0-dev` 每周合一次；`2.0-dev → main` 只在 2.0 发布时~~ —— **✅ 已完成**：
   v2.0.0 就是这条线合回来的结果（`2.0-dev` 曾是 `main` 的祖先，落后 51 个提交、0 个独有）。
   此后**不再设长驻开发线**，结构改动也直接进 `main`，靠"小步 + 每步带测试 + 发版前跑全量"控风险；
3. 外部 PR（如 #14）照常进 `main`；
4. **分支用完就删**：合并后的短分支、以及完成使命的开发线不再保留（`git branch -d` /
   `git push origin --delete`）。要长期留档一律用 **tag**，不要用分支 ——
   2026-09-19 那次身份事故的备份就是"分支 + tag 指向同一提交"，删掉分支零损失；
...
- **D25 行为要求已落地（2026-09-19，第 5 切片）**：ECHO AUTO 自动注册进 DSH 改为
  **仅在装了 `agent-dsh` 时执行**（没装就不碰 `~/.dsh/settings.yaml`，而路由本身照常可用；
  ...
  取舍与四条具体理由见 PROGRESS §42，安排与 P4 的组件布局一起做。
- **P5 面板已落地（2026-09-19，第 6 切片）**：设置页新增「能力 provider」卡片 ——
  三个 kind 各一个下拉（选中即写配置）、**出网标注显在脸上**、在线服务预设一键填入地址与
  模型名（密钥仍由用户填）；另有接线守卫测试防"卡片变孤儿"。视觉未经人眼确认（本环境无浏览器），
  见 PROGRESS §43 的诚实边界。
