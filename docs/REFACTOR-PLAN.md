...
| 交付包 | ≈8.8 GB / 6 万文件，解压 5–20 分钟 | `docs/新机器部署指南.md` |
| 模型入库 | `git ls-files models` = **0 条**（`models/` 已 gitignore） | git |

**必须先纠正的现状问题：**

1. **文档与现实不符**：`新机器部署指南.md` 称 KWS 唤醒词与 pyannote "已在 git 里"，
...
| **P8** | 发布与文档：双平台发布流水线（含 mac 签名/公证自动化与组件包公证策略）、重写 `DEPLOY.md` / `新机器部署指南.md` / `跨平台约定.md`、验收与回滚清单 | 4–6 天 | 同事在 Windows 与 macOS 上各按文档从零装完可用 |
...
| 交付包 8.8 GB 由作者手工提供（`docs/新机器部署指南.md`） | 唯一的"开箱可用"通道是人肉分发，不可自助 |
...
| Release 内容 | 说明 |
|---|---|
| 源码归档 | GitHub 自动提供（`/archive/refs/tags/v1.9.0.zip`），开发者可精确定版 |
| `SHA256SUMS` + 组件清单 | 校验交付物完整性 |
| **交付包获取说明** | 见下——注意 2 GiB 限制 |
| 升级说明 / 已知问题 | 从哪个版本升、要不要跑迁移 |

**基线动作**：现在给 `main` 打 **`v1.9.0`**（当前代码就是事实上的稳定版），
从此"当前版本"有了名字，2.0 走 `v2.0.0-rcN`。

### 13.4 交付包怎么分发（2 GiB 硬约束）

⚠️ **GitHub Release 单个 asset 上限 2 GiB**，现在 8.8 GB 的整包放不上去。三条路：

| 方案 | 做法 | 评价 |
|---|---|---|
| **(a) 整包外置**（推荐） | 8.8 GB 整包继续走内网共享盘 / 网盘；Release 里只放**版本说明 + sha256 + 获取链接** | 最省事；但"开箱可用"仍依赖人肉分发 |
| (b) 分卷上传 | 7z 分 5 卷传 Release | 可自助，但同事要按顺序解压，容易出错 |
| **(c) 等 2.0 的分层交付** | 主包 10–20 MB → **Release asset 完全放得下**；组件包按需单独发 | **D22 的又一个额外收益**：自助下载成为可能 |

所以：**2.0 之前走 (a)**，同时开始按 2.0 的分层方式产出交付物（1.x 仍是整包，但命名与
校验和先规范起来）。

### 13.5 三类人走三条路（要写进 README）

| 对象 | 拿到什么 | 怎么拿 |
|---|---|---|
| **同事（要能用）** | 稳定版交付包 | 走 Release 里的获取说明 / 内网共享盘；**不碰 git** |
| **开发者（看代码 / 提 PR）** | `git clone` → 默认 `main` = **稳定版** | 参与 2.0 才切 `2.0-dev` |
| **只要某一版源码** | Release 的源码归档 | `archive/refs/tags/vX.Y.Z.zip`，精确定版 |

README 的「快速开始」要拆成两段：**稳定版安装**（面向使用者）与
**从源码跑（开发者）**——现在两段混在一起且都是 clone 路线。
...
### 13.7 开工前必须先还的稳定性欠债（2026-09-18 实测）

本机现在有 **4 个 ECHO 进程，其中 2 个活着但不监听**：

| PID | 启动方式 | 监听 | 判断 |
|---|---|---|---|
| 35944 | uv python（经仓库 `venv`） | 18060 | ✅ 真正在服务 |
| 29208 | `.echo-venv`（`startup.ps1` 拉起） | 无 | ❌ 空转 |
| 22784 | uv python | 18061 | ✅ 路由 |
| 42196 | `.echo-venv` | 无 | ❌ 空转 |

（`.echo-venv` 是指向仓库 `venv` 的 junction，所以不是装了两份，而是**两条启动路径各拉起一次**。）

根因：

1. `scripts/startup.ps1` 把自己的单实例假设写在注释里——「ECHO has its own anti-duplicate
   guard (it exits when the port is taken), so this probe-then-start dance **can never race**
   with a live instance」。**该假设不成立**：存在"活着但没监听"的实例；
2. `app/main.py:161-171` 的守卫是"先 `connect_ex` 探端口、空闲才继续"——**探测与 uvicorn
   真正 bind 之间存在竞态窗口**，两个实例同时启动就会双双重入；
3. 路由被**两套机制**各拉一次：启动文件夹的 `ECHO 模型路由.lnk`
   （由 `dsh-failover/install-autostart.ps1` 安装）**和** ECHO 自己的 `boot.py` failover 组件；
4. `echo-supervisor.log` 显示 `startup.ps1` 当天被拉起 5 次，每次都启动一个新实例。

**为什么这与 2.0 直接相关**：任何基于端口的探活/重启判断都会被空转实例误导，届时你分不清
"坏的是新代码"还是"这套重复启动"。**必须先修**：

- 单实例改用**命名互斥量 / 文件锁**，不再依赖端口探测（探测天然竞态）；
- 启动前只做一次权威判定，失败即退；退出路径确保不留非 daemon 线程；
- 路由**只保留一个启动方**（推荐交给 `boot.py`，删掉启动文件夹那条）；
- 探活语义区分"进程存在"与"端口在听"。
...
1. 清空转实例 → 修单实例守卫（互斥量）→ 路由只留一个启动方（§13.7）；
2. 版本号收敛到单一来源 + `/api/status` 暴露版本 + 给 `main` 打 **`v1.9.0`** 并发首个 Release（§13.3）；
3. README 拆成"稳定版安装 / 从源码跑"两段 + 三类人分流（§13.5）；
4. 冻结稳定安装到 ASCII 路径，日常自用切过去（§13.6）；
5. 建 `2.0-dev`，把本文档提交进 `main`。
