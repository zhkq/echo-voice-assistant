# -*- coding: utf-8 -*-
"""Windows 全局热键与媒体键实现（纯 ctypes，无 C# 编译依赖）—— 平台专有，收在接缝里。

从 `app/hotkey.py` 整段搬来（P3 剩余搬迁）；`app/hotkey.py` 现在是门面，按平台
转发到本模块（Windows）或 `_posix_hotkey`（macOS/Linux）。业务代码只 import 门面。
